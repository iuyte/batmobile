...
def styleContent(commit):
    ...
    out += "<p><b style='cursor: pointer;' onclick='window.open(\"https://github.com/iuyte/batmobile/commit/" + commit.commitkey + "\")'>Commit:</b> <a class='link' href='https://iuyte.github.io/batmobile#" + commit.commitkey + "'>" + commit.commitkey + "</a></p>"
    ...
    if len(commit.filesAdded) > 0:
        ...
        for i in range(len(commit.filesAdded)):
            out += "<li><span class='link' onclick='window.open(\"https://github.com/iuyte/batmobile/tree/" + commit.commitkey + "/" + commit.filesAdded[i] + "\")'>" + commit.filesAdded[i] + "</span></li>\n"
        out += "</ul>"
    if len(commit.filesModified) > 0:
        out += "<b>Files modified:</b>\n<ul>\n"
        for i in range(len(commit.filesModified)):
            out += "<li><span class='link' onclick='window.open(\"https://github.com/iuyte/batmobile/tree/" + commit.commitkey + "/" + commit.filesModified[i] +"\")'>" + commit.filesModified[i] + "</span></li>\n"
        out += "</ul>"
